...
echo -e "${GREEN}===== NFS配置管理工具一键部署脚本 =====${NC}"
echo -e "${YELLOW}适用于openEuler操作系统${NC}"
echo

# 1. 创建工具目录
echo -e "${GREEN}1. 创建工具目录...${NC}"
...
# 4. 创建自动安装脚本
echo -e "${GREEN}\n4. 创建自动安装脚本...${NC}"
sudo cat > /opt/nfs-manager/install_prerequisites.sh << 'EOF'
#!/bin/bash

# 自动安装和配置NFS配置管理工具的前置条件
# 适用于openEuler操作系统
...
echo -e "${GREEN}===== 开始安装NFS配置管理工具前置条件 =====${NC}"
...
if ! grep -q "openEuler" /etc/os-release; then
    echo -e "${YELLOW}警告：当前操作系统不是openEuler，可能不兼容！${NC}"
...
echo -e "${GREEN}✓ 确认操作系统为openEuler${NC}"

# 2. 检查并安装nfs-utils
echo -e "${GREEN}\n===== 安装nfs-utils软件包 =====${NC}"
if rpm -q nfs-utils >/dev/null 2>&1; then
    echo -e "${YELLOW}nfs-utils已经安装，跳过...${NC}"
else
    echo -e "${GREEN}正在安装nfs-utils...${NC}"
    sudo dnf install -y nfs-utils
    echo -e "${GREEN}✓ nfs-utils安装完成${NC}"
fi
...
if [ "$PYTHON_MAJOR" -lt 3 ] || ([ "$PYTHON_MAJOR" -eq 3 ] && [ "$PYTHON_MINOR" -lt 11 ]); then
    echo -e "${YELLOW}警告：Python版本 ${PYTHON_VERSION} 低于推荐的3.11版本${NC}"
else
    echo -e "${GREEN}✓ Python版本 ${PYTHON_VERSION} 符合要求${NC}"
fi

# 4. 检查并安装pip
echo -e "${GREEN}\n===== 检查pip =====${NC}"
if ! command -v pip3 &> /dev/null; then
    echo -e "${GREEN}正在安装pip...${NC}"
...
# 5. 安装rich库
echo -e "${GREEN}\n===== 安装Python依赖 =====${NC}"
echo -e "${GREEN}正在安装rich库...${NC}"
sudo pip3 install rich
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ rich库安装完成${NC}"
else
    echo -e "${RED}错误：rich库安装失败！${NC}"
    exit 1
fi

# 6. 检查工具文件
echo -e "${GREEN}\n===== 检查工具文件 =====${NC}"
if [ ! -f "/opt/nfs-manager/nfs_manager.py" ]; then
    echo -e "${RED}错误：未找到nfs_manager.py文件！${NC}"
    echo -e "${YELLOW}请确保该文件存在于/opt/nfs-manager/目录下${NC}"
    exit 1
else
    echo -e "${GREEN}✓ 找到nfs_manager.py文件${NC}"
fi

# 7. 设置执行权限
echo -e "${GREEN}\n===== 设置执行权限 =====${NC}"
sudo chmod +x /opt/nfs-manager/nfs_manager.py
echo -e "${GREEN}✓ 已设置nfs_manager.py执行权限${NC}"

# 8. 显示安装结果
echo -e "${GREEN}\n===== 安装完成 =====${NC}"
echo -e "${GREEN}✓ 所有前置条件已安装配置完成！${NC}"
echo -e "${YELLOW}\n使用说明：${NC}"
echo -e "${YELLOW}1. 查看帮助：${NC} python3 /opt/nfs-manager/nfs_manager.py --help"
echo -e "${YELLOW}2. 交互式模式：${NC} sudo python3 /opt/nfs-manager/nfs_manager.py"
echo -e "${YELLOW}3. 命令行模式：${NC} sudo python3 /opt/nfs-manager/nfs_manager.py [参数]"
echo -e "${YELLOW}4. 查看使用指南：${NC} cat /opt/nfs-manager/使用指南.md"

echo -e "${GREEN}\n===== 安装脚本执行结束 =====${NC}"
EOF
echo -e "${GREEN}✓ 自动安装脚本创建完成${NC}"

# 5. 设置执行权限
echo -e "${GREEN}\n5. 设置执行权限...${NC}"
sudo chmod +x /opt/nfs-manager/*.sh
echo -e "${GREEN}✓ 脚本执行权限设置完成${NC}"

# 6. 安装前置条件
echo -e "${GREEN}\n6. 安装前置条件...${NC}"
sudo /opt/nfs-manager/install_prerequisites.sh

# 7. 显示使用说明
echo -e "${GREEN}\n===== 部署完成 =====${NC}"
echo -e "${GREEN}✓ NFS配置管理工具已成功部署！${NC}"
echo -e "${YELLOW}\n使用方法：${NC}"
echo -e "${YELLOW}1. 查看帮助信息：${NC} sudo python3 /opt/nfs-manager/nfs_manager.py --help"
echo -e "${YELLOW}2. 进入交互式管理界面：${NC} sudo python3 /opt/nfs-manager/nfs_manager.py"
echo -e "${YELLOW}3. 直接使用命令行参数：${NC} sudo python3 /opt/nfs-manager/nfs_manager.py --show-status"
